...
type GenerateRequest struct {
	Goal  string `json:"goal"`  // hypertrophy|strength|fatloss
...
func GenerateTreino(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Level == "" { req.Level = "beginner" }
		if req.Days <= 0 { req.Days = 3 }
		if req.Split == "" { req.Split = "push-pull-legs" }
		...
		for i, focus := range foci {
			groupNames := groupsForFocusPT(focus)
...
func buildSplit(split string, days int) []string {
	...
	case "push-pull-legs":
		...
		for i := 0; i < days; i++ { out = append(out, seq[i%3]) }
		return out
	case "upper-lower":
		seq := []string{"upper", "lower"}
		out := make([]string, 0, days)
		for i := 0; i < days; i++ { out = append(out, seq[i%2]) }
		return out
	case "full-body":
		out := make([]string, days)
		for i := range out { out[i] = "full" }
		return out
	default:
		return nil
	}
